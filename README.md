...
`libzucrypt.a` for one that cannot. The archive is provisional until its
first consumer links it, and the table is experimental; see
`?zucrypt_c_api`.

## Status

Version 0.1.0, in development. The six R functions are stable; the C
interface is provisional (the archive) or experimental (the table) until a
consumer has linked it. The design is in `.agents/design.md`; `.agents/roadmap.md` records
