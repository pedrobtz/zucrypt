...
backend is a vendored, pinned subset of the Mbed TLS ecosystem, so
installing the package needs a C compiler and nothing else — no system
cryptographic library, no Java, no Python, and no network access during
installation.
...
the first release in `.agents/roadmap.md`; the R and C interfaces
described there are not implemented yet.
